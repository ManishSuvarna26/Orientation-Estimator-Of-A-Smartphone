...
%    -0.0001    0.0002    0.0412];
    Ra = [0.1362    0.0711   -0.0002
          0.0711    0.0790    0.0014
        - 0.0002    0.0014    0.0775];

%   Rg = 1.0e-05 *[0.0817    0.0017   -0.0026;
%                  0.0017    0.1054    0.0008;
%                 -0.0026    0.0008    0.0610];
    Rg = [0.0043   -0.0007   -0.0049
         -0.0007    0.0015    0.0131
         -0.0049    0.0131    0.6355];
  Rm = [31.4348  -14.5106   -1.5389
       -14.5106  104.8704   -0.9395
       -1.5389   -0.9395    4.1759];
  g0 = [0.0939 -0.1733 9.8425]';
  acc_err_margin = norm(g0)*0.2;
  m0 = [0 24.8054 -22.5217]';
   L = norm(m0);
%   mag_err_margin = norm(m0)*0.1;
...
%         if abs(norm(acc) - norm(g0))< acc_err_margin
